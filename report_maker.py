"""Report-maker: To generate pdf reports with tables.

Example usage (First copy this script to any valid python path):

>>> from report_maker import PdfReport, DataTable
>>> pr = PdfReport("Report title", "Company Name")
>>> dt1 = DataTable([1*inch, 3*inch, 1*inch, 3*inch], True)
>>> dt1.add_rows([
...     ('List of customers', '', '', ''),
...     ('Sl.No', 'Name', 'Age', 'Profession'),
...     (1, 'William Shapkeaspear', 23, 'Novelist'),
...     (2, 'Bernard Shaw', 42, 'Lawyer'),
... ])
>>> # Center-align text in all cells of first row 
>>> dt1.add_style(('ALIGN', (0, 0), (-1, 0), 'CENTER'))
>>> # Span the table title along the whole first row.
>>> dt1.add_style(('SPAN', (0, 0), (-1, 0)))
>>> # Center-align text in all other cells as well.
>>> dt1.add_style(('ALIGN', (0, 1), (-1, 1), 'CENTER'))
>>> # Create as much tables you want like this and build the doc
>>> # passing a list of all tables.
>>> pdf_doc = pr.build_doc([dt1])
>>> # Now, save it to file OR send it by mail OR generate an HTTP response
